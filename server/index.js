...
const allowedOrigins = [
    ...
    "https://chrisbridewell.dev",
    "http://chrisbridewell.dev",
];
app.use(
    cors({
...
)}@cluster0.kktwa.mongodb.net/${encodeURI(
...
app.ws("/:id?", function(ws, req) {
    ...
    ws.on("message", (message) => {
        ...
        if (parsedMessage.leaving) {
            listeningPlayers = listeningPlayers.filter(
                (p) => p.senderId != parsedMessage.senderId
            );
            return;
        }
        const { notification, key, playerId } = parsedMessage;
        const listeningPlayer = listeningPlayers.find(
            (p) => p.senderId == playerId
        );
